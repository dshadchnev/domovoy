...
public class DashboardRepositoryTests
{
    ...
    [Fact]
    public async Task GetTelemetryHistoryAsync_ReturnsPointsFromRedis()
    {
        // Arrange
        using var db = new DashboardDbContext(_dbOptions);
        var now = DateTime.UtcNow;
        var p1 = JsonSerializer.Serialize(new { Timestamp = now.AddMinutes(-10), Data = new Dictionary<string, object> { { "temperature", 22.5 } } });
        var p2 = JsonSerializer.Serialize(new { Timestamp = now.AddMinutes(-5), Data = new Dictionary<string, object> { { "temperature", 25.0 } } });

        _redisDbMock.Setup(r => r.ListRangeAsync("device:telemetry:history:dev-01", 0, -1, CommandFlags.None))
            .ReturnsAsync(new RedisValue[] { p1, p2 });

        var repo = new DashboardRepository(db, _redisMock.Object, _loggerMock.Object);

        // Act
        var history = await repo.GetTelemetryHistoryAsync("dev-01", now.AddMinutes(-15), now);

        // Assert
        Assert.Equal("dev-01", history.DeviceId);
