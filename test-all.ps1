...
try {
    .\Test-E2E.ps1 -Cleanup:$false | Out-Null
...
$results["Gateway Routing"] = if($gatewayOk){"✅ Pass"}else{"⚠️ Partial"}
...
$results["Event Bus"] = if($eventsOk){"✅ Pass"}else{"⚠️ Pending"}
...
Write-Host "`n$(if($allPass){'🎉 ALL TESTS PASSED'}else{'⚠️  Some tests need attention'})" -ForegroundColor $(if($allPass){"Cyan"}else{"Yellow"})